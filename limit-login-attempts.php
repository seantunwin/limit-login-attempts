...
$limit_login_level_role = array(
	0	 => __( 'Subscriber', 'limit-login-attempts' ),
...
function limit_login_setup() {
	load_plugin_textdomain( 'limit-login-attempts', PLUGINDIR . '/' . dirname( plugin_basename( __FILE__ ) ) );
...
function limit_login_check_time( $check_array, $ip = null ) {
	...
	return ( is_array( $check_array ) && isset( $check_array[ $ip ] ) && time() <= $check_array[ $ip ] );
}
...
function is_limit_login_reg_ok() {
	...
	return ( ! limit_login_check_time( $valid, $ip ) || ! isset( $regs[ $ip ] ) || $regs[ $ip ] < $allowed );
}
...
function limit_login_handle_cookies() {
	...
	if ( empty( $_COOKIE[AUTH_COOKIE] ) && empty( $_COOKIE[SECURE_AUTH_COOKIE] ) && empty( $_COOKIE[LOGGED_IN_COOKIE] ) ) {
		return;
	}
...
function limit_login_add_error_message() {
	...
	if ( is_limit_login_reg_page() && ! is_limit_login_reg_ok() && ! $limit_login_my_error_shown ) {
...
function limit_login_sanitize_options() {
	...
	$cookies = limit_login_option( 'cookies' ) && limit_login_support_cookie_option() ? true : false;

	$limit_login_options['cookies'] = $cookies;

	if ( limit_login_option( 'client_type' ) != LIMIT_LOGIN_DIRECT_ADDR && limit_login_option( 'client_type' ) != LIMIT_LOGIN_PROXY_ADDR ) {
		$limit_login_options['client_type'] = LIMIT_LOGIN_DIRECT_ADDR;
	}

	$pwd_reset_func_supported = limit_login_support_pwd_reset_options();
	$pwd_reset_username = limit_login_option( 'disable_pwd_reset_username' ) && $pwd_reset_func_supported;
	$pwd_reset = limit_login_option( 'disable_pwd_reset' ) && $pwd_reset_func_supported;
...
function limit_login_guess_proxy() {
	return isset( $_SERVER[LIMIT_LOGIN_PROXY_ADDR] ) ? LIMIT_LOGIN_PROXY_ADDR : LIMIT_LOGIN_DIRECT_ADDR;
}
...
function limit_login_show_users() {
	...
	if ( ! $bad_count ) {
		echo( sprintf( '<p><i>%s</i></p>', __( "Privileged usernames, display names, url names and nicknames are ok", 'limit-login-attempts' ) ) );
	}
